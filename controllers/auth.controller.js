  ...
  try {
    ...
    if (!passwordRegex.test(password)) {
      return res.status(400).json({
        message:
          "Password must be at least 8 characters long, include 1 uppercase, 1 lowercase, 1 number and 1 special character (e.g., Keshav@123)",
      });
    }
    ...
    // bcrypting the password before saving in database
    const hashedPassword = await bcrypt.hash(password, 10);
...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    // Password validation regex (example: Keshav@123)
    const passwordRegex =
      /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/;
    if (!passwordRegex.test(password)) {
      return res.status(400).json({
    ...
    // generating jwt token in case email and password matches
    const token = generateToken(user._id);

    // storing token in cookie
    res.cookie("token", token, {
      httpOnly: true,
      sameSite: "None", // Or 'None' if you're testing cross-origin + HTTPS
      secure: true, // Set to true in production with HTTPS
      maxAge: 7 * 24 * 60 * 60 * 1000, // 7 days
    });
